...
import {
  ...
  fullRosterAutoCancelDeadlineUtcMs,
  formatDurationUntil,
...
export function MatchDetailScreen({
  ...
  const schedulePayloadForDeadline = {
    date: matchDtoDateIso(match),
    timeLabel: match.timeLabel ?? "00:00",
    isInstant: Boolean(match.isInstant),
  };
  const slotPastForSchedule = scheduledNonInstantSlotIsExpired(schedulePayloadForDeadline);
  const fullAutoCancelDeadline = fullRosterAutoCancelDeadlineUtcMs({
    date: matchDtoDateIso(match),
    timeLabel: match.timeLabel ?? "00:00",
    isInstant: Boolean(match.isInstant),
  });
  const nowMsDetail = Date.now();
  const showFullRosterDeadlineBanner =
    status === "full" &&
    !match.isInstant &&
    slotPastForSchedule &&
    Number.isFinite(fullAutoCancelDeadline) &&
    nowMsDetail < fullAutoCancelDeadline;
  const fullRosterTimeLeftLabel = showFullRosterDeadlineBanner
    ? formatDurationUntil(fullAutoCancelDeadline, nowMsDetail)
    : "";
  ...
  return (
    ...
    >
      ...
      {showFullRosterDeadlineBanner ? (
        <View style={styles.fullRosterDeadlineBanner} accessibilityRole="alert">
          <Text style={styles.fullRosterDeadlineTitle}>Start this match soon</Text>
          <Text style={styles.fullRosterDeadlineText}>
            The roster is full and the scheduled time has passed. If the organiser does not start the match, it will be
            cancelled automatically in about {fullRosterTimeLeftLabel}. Everyone on the roster will be notified.
          </Text>
        </View>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  fullRosterDeadlineTitle: { fontSize: 14, fontWeight: "800", color: COLORS.infoText },
  fullRosterDeadlineText: { fontSize: 12, color: COLORS.text, marginTop: 6, lineHeight: 17 },
  disputeBanner: {
